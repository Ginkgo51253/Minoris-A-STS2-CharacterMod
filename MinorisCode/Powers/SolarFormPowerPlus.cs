...
能力中文名称: 太阳形态+
能力英文名称: Solar Form+
能力描述(ZHS): 在你的回合开始时，随机将升级过的攻击牌、技能牌、能力牌各一张置入你的手中。它们在本场战斗中为0费，但具有虚无。
能力描述(ENG): At the start of your turn, add an upgraded random Attack, Skill, and Power to your hand. They cost 0 this combat, but are Ethereal.
相关卡牌（本地键）: MINORIS-CARD071_SOLAR_FORM
*/
public class SolarFormPowerPlus : MinorisPower
{
    ...
    public override async Task AfterPlayerTurnStart(PlayerChoiceContext choiceContext, Player player)
    {
        ...
        {
            await CreateAndAdd(pickA, true);
        }
        for (var i = 0; i < countPerType; i++)
        {
            await CreateAndAdd(pickS, true);
        }
        for (var i = 0; i < countPerType; i++)
        {
